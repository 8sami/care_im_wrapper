...
from care_im_wrapper.conversation.handlers import _handle_authenticated, _handle_selecting_document
...
class EnterDocumentSelectionTests(TestCase):
    ...
    def test_offers_one_row_per_record_and_parks_in_selecting_document(self, mock_send, mock_resolve_actor, _max):
        ...
        with self._patch_menu(records):
            _handle_authenticated(self.session, PHONE, "5", CHANNEL)
        ...
        rows = mock_send.call_args[0][2].interactive.action_data[0]["rows"]
    ...
    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_caps_rows_so_the_back_row_survives_the_providers_row_limit(
        self, mock_send, mock_resolve_actor, _max, _rows
    ):
    ...
    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_records_without_an_external_id_fall_back_to_the_plain_text_reply(
        self, mock_send, mock_resolve_actor, _max
    ):
    ...
    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_summary_too_long_for_the_body_keeps_the_rows_and_drops_the_dump(
        self, mock_send, mock_resolve_actor, _max, _limit
    ):
        ...
        self.assertEqual(msg.interactive.body, "Which one would you like the document for?")
...
class HandleSelectingDocumentTests(TestCase):
    ...
    def test_selecting_a_row_sends_a_cta_button_for_that_record(
        self, mock_send, mock_build_url, mock_get_link, mock_resolve_patient, mock_resolve_actor
    ):
        mock_resolve_actor.return_value = _make_actor()
        ...
        mock_send.assert_called_once()
        document_msg = mock_send.call_args[0][2]
    ...
    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_typed_number_selects_the_same_row_as_the_interactive_id(self, mock_send, mock_resolve_actor):
        mock_resolve_actor.return_value = _make_actor()
        ...
        with (
            ...
            _handle_selecting_document(self.session, PHONE, "1", CHANNEL)

        document_resolver.assert_called_once_with(mock_patient.return_value, "uuid-1")

    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_back_returns_to_the_main_menu_without_generating(self, mock_send, mock_resolve_actor):
        mock_resolve_actor.return_value = _make_actor()
        document_resolver = MagicMock()

        with self._patch_menu(document_resolver):
            _handle_selecting_document(self.session, PHONE, "0", CHANNEL)

        document_resolver.assert_not_called()
        self.session.refresh_from_db()
        self.assertEqual(self.session.state, ConversationSession.State.AUTHENTICATED)
        mock_send.assert_called_once()
        self.assertEqual(mock_send.call_args[0][2].interactive.type, InteractiveType.LIST)

    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.resolve_target_patient")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_record_with_no_document_reports_back_and_returns_to_menu(
        self, mock_send, mock_resolve_patient, mock_resolve_actor
    ):
        mock_resolve_actor.return_value = _make_actor()
        document_resolver = MagicMock(return_value=None)
        ...
        mock_send.assert_called_once()
        self.assertIn("isn't available", mock_send.call_args[0][2].text)
        self.session.refresh_from_db()
        self.assertEqual(self.session.state, ConversationSession.State.AUTHENTICATED)
    ...
    def test_generation_failure_reports_back_and_returns_to_menu(
        self, mock_send, mock_get_link, mock_resolve_patient, mock_resolve_actor
    ):
        mock_resolve_actor.return_value = _make_actor()
    ...
    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_out_of_range_choice_stays_in_selection(self, mock_send, mock_resolve_actor):
        mock_resolve_actor.return_value = _make_actor()

        with self._patch_menu(MagicMock()):
            _handle_selecting_document(self.session, PHONE, "document_7", CHANNEL)
