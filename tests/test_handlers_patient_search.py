...
from care_im_wrapper.conversation.handlers import _handle_awaiting_patient_search
...
class HandleAwaitingPatientSearchTests(TestCase):
    ...
    def test_actor_none_logs_out_without_calling_search(self, mock_send, mock_search, mock_resolve_actor):
        _handle_awaiting_patient_search(self.session, PHONE, "Jane", CHANNEL)
    ...
    @patch("care_im_wrapper.conversation.handlers.patient_lookup.search_patients")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_permission_denied_reverts_to_authenticated_state(self, mock_send, mock_search, mock_resolve_actor):
        ...
        mock_send.assert_called_once_with(CHANNEL, PHONE, "You don't have permission to view this information.")

    @patch("care_im_wrapper.conversation.handlers.resolve_actor")
    @patch("care_im_wrapper.conversation.handlers.patient_lookup.search_patients")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_no_data_error_sends_no_patients_found_and_leaves_state_unchanged(
        self, mock_send, mock_search, mock_resolve_actor
    ):
        mock_resolve_actor.return_value = _make_actor()
        mock_search.side_effect = NoDataError("nothing found")

        _handle_awaiting_patient_search(self.session, PHONE, "Zzz", CHANNEL)

        self.session.refresh_from_db()
        self.assertEqual(self.session.state, ConversationSession.State.AWAITING_PATIENT_SEARCH)
        mock_send.assert_called_once_with(CHANNEL, PHONE, "No patients found matching that search.")
    ...
    def test_three_or_fewer_results_uses_reply_buttons_and_saves_candidates(
        self, mock_send, mock_search, mock_resolve_actor, mock_max_chars
    ):
        mock_resolve_actor.return_value = _make_actor()
        results = [
        ...
        _handle_awaiting_patient_search(self.session, PHONE, "Doe", CHANNEL)
    ...
    @patch("care_im_wrapper.conversation.handlers.patient_lookup.search_patients")
    @patch("care_im_wrapper.conversation.handlers.send_message")
    def test_more_than_three_results_uses_list_with_descriptions(
        self, mock_send, mock_search, mock_resolve_actor, mock_max_chars
    ):
        mock_resolve_actor.return_value = _make_actor()
        results = [
        ...
        _handle_awaiting_patient_search(self.session, PHONE, "Patient", CHANNEL)

        call_msg = mock_send.call_args[0][2]
